...
function addGamesToPage(games) {
    ...
    for (var game in games) {
        ...
        let name = games[game].name;
        let description = games[game].description;
        let pledged = games[game].pledged;
...
let gamePlural = numUnfundedGames == 1 ? "game remains" : "games remain";
const displayStr = `A total of $${totalRaised.toLocaleString('en-US')} has been raised for ${totalGames - numUnfundedGames} games. Currently, ${numUnfundedGames} ${gamePlural} unfunded. We need your help to fund these amazing games!`;
...
[firstGame, secondGame] = [sortedGames[0], sortedGames[1]];

// create a new element to hold the name of the top pledge game, then append it to the correct element
let p1 = document.createElement('h1');
p1.innerHTML = `${firstGame.name}`;
firstGameContainer.append(p1);

// do the same for the runner up item
let p2 = document.createElement('h1');
p2.innerHTML = `${secondGame.name}`;
secondGameContainer.append(p2);